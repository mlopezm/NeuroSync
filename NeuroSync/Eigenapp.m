...
if eigenA(end-1)>low_val 
    ind_A=eigenA(1)/eigenA(end-1);
else
    ind_A='L2 is zero';
end;
if  eigenAw(end-1)>low_val  
    ind_Aw=eigenAw(1)/eigenAw(end-1);
else
    ind_Aw='L2 is zero';
end; 
if  eigenL(end-1)>low_val  
    ind_L=eigenL(1)/eigenL(end-1);
else
    ind_L='L2 is zero';
end;
if  eigenLw(end-1)>low_val  
    ind_Lw=eigenLw(1)/eigenLw(end-1);
else
    ind_Lw='L2 is zero';
end;
...
if ssA>low_val 
    ...
    sigma_A='Sum matrix (out of diagonal) is zero';
end;

t0=sum(sum(S))-sum(diag(S));
ssAw=t0/size(S,1);
t1=sum(eigenLw(1:end-1));
avgEigLw=t1/(size(S,1)-1);
if  ssAw>low_val  
    ...
    sigma_Aw='Sum matrix (out of diagonal) is zero';
end; 
